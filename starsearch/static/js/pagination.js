	...
	return this.each(function() {
		...
		function drawLinks() {
			...
			if (opts.display_msg) {
				if(!maxentries){
					panel
						.append('<div class="showing_stats">Sorry, we found nothing.</div>');
				}else{
				panel
						.append('<div class="showing_stats">Showing results of No.'
								+ ((current_page * opts.items_per_page) + 1)
								+ ' to No.'
								+ (((current_page + 1) * opts.items_per_page) > maxentries
										? maxentries
										: ((current_page + 1) * opts.items_per_page))
								+ (maxentries > 1 ? ('. There are ' + maxentries + ' results in total.')
									: ('. Found ' + maxentries + ' result.')) + '</div>');
				}
			}

			if(opts.setPageNo){
				  panel.append("<div class='goto'><span class='text'>Jump to Page </span><input type='text'/><button type='button' class='ue-button long2'>Yes</button></div>");
			}
		}
		...
		if(maxentries==0){
			panel.append('<span class="current prev">'+opts.prev_text+
				'</span><span class="current next">'+opts.next_text+
				'</span><div class="showing_stats">Sorry, we found nothing.</div>');
		}else{
			drawLinks();
		}
